...
export class PipingTools {
     ...
     */
    static async pipe(payload, options = {}) {
        // Validate payload
     ...
     */
    static async pipeToPlugin(payload, pluginUrl, pipeName, configuration) {
     ...
     */
    static async pipeBroadcast(payload, pipeName) {
        return this.pipe(payload, { name: pipeName });
    }
    /**
     * Send data with advanced action pipe options
     */
    static async actionPipe(payload, options = {}) {
        // Validate payload
        const textValidation = Validator.validateText(payload);
        if (!textValidation.valid) {
            throw new ValidationError(`Invalid payload: ${textValidation.errors.join(', ')}`);
     ...
     */
    static async pipeWithResponse(payload, options = {}) {
     ...
     */
    static async pipeFromFile(filePath, options = {}) {
        // Validate file path
