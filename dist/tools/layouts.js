...
export class LayoutTools {
     ...
     */
    static async dumpLayout(outputPath) {
     ...
     */
    static async saveLayout(layoutName, layoutsDir) {
     ...
     */
    static async applyLayout(layoutName, sessionName) {
        // Validate layout name
        const nameValidation = Validator.validateString(layoutName, 'layout name', 64);
        if (!nameValidation.valid) {
            throw new ValidationError(`Invalid layout name: ${nameValidation.errors.join(', ')}`);
     ...
     */
    static async listLayouts(layoutsDir) {
        // Check cache first
     ...
     */
    static async loadLayout(layoutName, layoutsDir) {
        // Validate layout name
        const nameValidation = Validator.validateString(layoutName, 'layout name', 64);
        if (!nameValidation.valid) {
            throw new ValidationError(`Invalid layout name: ${nameValidation.errors.join(', ')}`);
     ...
     */
    static async newTabWithLayout(layoutName, tabName) {
        // Validate layout name
        const nameValidation = Validator.validateString(layoutName, 'layout name', 64);
        if (!nameValidation.valid) {
            throw new ValidationError(`Invalid layout name: ${nameValidation.errors.join(', ')}`);
     ...
     */
    static async convertLayout(inputPath, outputPath, fromFormat, toFormat) {
        // Validate paths
     ...
     */
    static convertKdlToJson(kdlContent) {
     ...
     */
    static convertJsonToKdl(jsonData) {
     ...
     */
    static async validateLayout(layoutPath) {
        // Validate path
