...
import { ToolResponse, SessionExport, ZellijLayout, ValidationError } from '../types/zellij.js';
...
export class SessionTools {
   ...
   */
  static async listSessions(): Promise<ToolResponse> {
    // Check cache first
   ...
   */
  static async getSessionInfo(sessionName: string): Promise<ToolResponse> {
   ...
   */
  static async exportSession(sessionName: string, outputPath?: string): Promise<ToolResponse> {
    // Validate session name
    const nameValidation = Validator.validateSessionName(sessionName);
    if (!nameValidation.valid) {
      throw new ValidationError(`Invalid session name: ${nameValidation.errors.join(', ')}`);
   ...
   */
  static async importSession(importPath: string, newSessionName?: string): Promise<ToolResponse> {
    ...
    try {
      ...
      const sessionData: SessionExport = JSON.parse(importData);
   ...
   */
  static async cloneSession(sourceSessionName: string, newSessionName: string): Promise<ToolResponse> {
   ...
   * Rename a session
   */
  static async renameSession(oldName: string, newName: string): Promise<ToolResponse> {
    // Validate session names
   ...
   * Switch to a session
   */
  static async switchSession(sessionName: string): Promise<ToolResponse> {
    // Validate session name
    const nameValidation = Validator.validateSessionName(sessionName);
    if (!nameValidation.valid) {
      throw new ValidationError(`Invalid session name: ${nameValidation.errors.join(', ')}`);
   ...
   */
  private static convertJsonToKdl(layout: any): string {
   ...
   * Kill all sessions
   */
  static async killAllSessions(): Promise<ToolResponse> {
   ...
   * Delete all sessions
   */
  static async deleteAllSessions(): Promise<ToolResponse> {
